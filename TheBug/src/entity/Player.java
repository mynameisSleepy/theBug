	...
	public Player(TileMap tm) {
		...
		*/
		...
		try {
			...
			BufferedImage attackImage = ImageIO.read(
					getClass().getResourceAsStream(
						"/Sprites/Player/PlayerSprite_Attack.png"
					)
				);
			
			BufferedImage[] bi = new BufferedImage[1];
			BufferedImage[] biAttack = new BufferedImage[1];
			bi[0] = spritesheet.getSubimage(0, 0, width, height);
			biAttack[0] = attackImage.getSubimage(0, 0, width*2, height);
			sprites = new ArrayList<BufferedImage[]>();
			for(int i = 0; i < 7; i++) {
				if(i != SCRATCHING)
					sprites.add(bi);
				else
					sprites.add(biAttack);
			}
			/*
			sprites = new ArrayList<BufferedImage[]>();
			for(int i = 0; i < 7; i++) {
